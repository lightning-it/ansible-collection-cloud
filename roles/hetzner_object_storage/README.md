...
Classification: configuration as code. This is the canonical
`lit.cloud.hetzner_object_storage` implementation. Basic contract coverage is
repository-local; Heavy and Application Acceptance are centrally owned by
...
`hetzner_object_storage_action`, `hetzner_object_storage_endpoint`,
`hetzner_object_storage_region`, `hetzner_object_storage_bucket`,
`hetzner_object_storage_prefix`,
the Object Lock and lifecycle settings, and exactly one credential source.
Actions are `plan`, `audit`, and `reconcile`.

`hetzner_object_storage_bucket_project_id` identifies the project that owns the
bucket. `hetzner_object_storage_external_principals` is an optional list whose
...
- name: Reconcile one protected Hetzner S3 bucket
  ...
  gather_facts: false
  roles:
    - role: lit.cloud.hetzner_object_storage
      vars:
        hetzner_object_storage_action: reconcile
        hetzner_object_storage_endpoint: https://fsn1.your-objectstorage.com
        hetzner_object_storage_region: fsn1
        hetzner_object_storage_bucket: example-production-backup
        hetzner_object_storage_prefix: host01
        hetzner_object_storage_bucket_project_id: "10001"
        hetzner_object_storage_external_principals:
          - name: backup-writer
            project_id: "20001"
            access_key: "{{ protected_writer_access_key }}"
            profile: writer
        hetzner_object_storage_use_vault: true
        hetzner_object_storage_vault_addr: https://vault.example.com
        hetzner_object_storage_vault_token: "{{ protected_vault_token }}"
        hetzner_object_storage_vault_kv_mount: infrastructure
        hetzner_object_storage_vault_kv_path: object-storage/host01
```
